...
(function() {
  ...
  DashboardController.$inject = ['$scope', 'ChartData', '$timeout', '$localStorage', 'API'];

  function DashboardController($scope, ChartData, $timeout, $localStorage, API) {
    var vm = this;
    var splinedata2 = [{
      "label": "Array Voltage",
      "color": "#768294",
    ...
    function activate() {
      ...
      // SPLINE
      // ----------------------------------- 
      API.getData(50, $localStorage.session_id)
        .then(function(response) {

            angular.forEach(response.data.resource, function(d) {

              ts = d.timestamp;
              date = new Date(ts * 1000);

              //console.log('***************** Showing Data***************')  
              //console.log(d.pvvoltage);
              splinedata2[0].data.push([date.getHours() + ":" + date.getMinutes(), d.pvvoltage]);

            });

            //console.log(splinedata2);

            vm.splineData = splinedata2;
            ...
            vm.splineOptions = {
              ...
              yaxis: {
                min: 0,
                max: 600, // optional: use it for a clear represetation
                tickColor: '#eee',
                position: ($scope.app.layout.isRTL ? 'right' : 'left'),
                tickFormatter: function(v) {
                  return v /* + ' visitors'*/ ;
...
(function() {
  ...
  angular
    .module('app.forms')
    .controller('AddRemoveDashboardController', AddRemoveDashboardController);

  AddRemoveDashboardController.$inject = ['$rootScope', '$scope', 'editableOptions', 'editableThemes', '$filter', '$http', '$localStorage', 'API', '$q', 'SidebarLoader'];

  function AddRemoveDashboardController($rootScope, $scope, editableOptions, editableThemes, $filter, $http, $localStorage, API, $q, SidebarLoader) {
    var vm = this;
    vm.title = 'Controller';
    //API to Get projects from the server. 
    ...
    function activate() {
      ...
      if ($localStorage.user_dashboards != null) {
        vm.dashboards = $localStorage.dashboards.slice();
        vm.current_dashboards = $localStorage.user_dashboards.slice();
        vm.modified_dashboards = vm.current_dashboards.slice();
      }

      vm.xyz = true;

      vm.delete = function(id) {
        if (vm.current_dashboards != null) {
          vm.current_dashboards.splice(vm.current_dashboards.indexOf(id), 1);
        }
      };
      ...
      vm.save = function() {
        ...
        API.patchDashboard($localStorage.session_id, $localStorage.user_id, vm.modified_dashboards)
          .then(function(response){
            $rootScope.$emit('CallLoadSidebar', {});
            /*API.refresh($localStorage.session_id)
              .then(function(response) {
                console.log("success refresh!");
                $rootScope.$emit('CallLoadSidebar', {});
              }, function() {
                console.log('Failed to refresh tables from Server');
              })*/
          }, function() {
            console.log('Failed to get user dashboard from Server');
          });
      }

      vm.cancel = function() {
        vm.current_dashboards = vm.modified_dashboards.slice();
      }

      vm.add = function() {
        var new_dash = true;
        if (vm.current_dashboards != null) {
          for (var i in vm.dashboards) {
            for (var j in vm.current_dashboards) {
              new_dash = vm.dashboards[i]._id == vm.current_dashboards[j] ? false : true;
              if (!new_dash) { break; }
            }
            if (new_dash) {
              vm.current_dashboards.push(vm.dashboards[i]._id);
...
(function() {
  ...
  FormxEditableController.$inject = ['$scope', 'editableOptions', 'editableThemes', '$filter', '$http', '$localStorage', 'API', '$q', '$state'];

  function FormxEditableController($scope, editableOptions, editableThemes, $filter, $http, $localStorage, API, $q, $state) {
    var vm = this;
    vm.title = 'Controller';
    var projects_limit = 3;
    vm.added_ids = [];
    vm.deleted_ids = [];
    vm.added_projects = [];
    vm.deleted_projects = [];
    ...
    function activate() {
      ...
      API.getUserProjects($localStorage.email, $localStorage.session_id, projects_limit)
        .then(function(response) {
          console.log('Got Projects from Server');

          vm.serverdata = response.data.resource;

          vm.user_projects = new Array(vm.serverdata.length);
          for (var i = 0; i < vm.user_projects.length; i++) {
            var project = vm.serverdata[i];
            ...
            for (var j = 0; j < keys.length; j++) {
              if (keys[j] == "_id") {
                vm.user_projects[i].id = vm.serverdata[i][keys[j]];
                continue;
              } else if (keys[j] == "project_name") {
                vm.user_projects[i].project_name = vm.serverdata[i][keys[j]];
              } else {
                vm.user_projects[i].keys.push(keys[j]);
                vm.user_projects[i].fields.push(vm.serverdata[i][keys[j]]);
              }
            }
          }
          console.log(vm.user_projects);
          vm.asdasd = function() {
            alert(1);
          };

          API.getAllProjects($localStorage.session_id)
            .then(function(response) {
              $localStorage.projects = response.data.resource.slice();
              vm.projects = $localStorage.projects.slice();
              vm.selected = Array.apply(null, {length: vm.projects.length}).map(function(){ return false; });
              for (var i in vm.projects) {
                vm.user_projects.forEach(function(p_user) {
                  if (p_user.id === vm.projects[i]._id) {
                    vm.selected[i] = true;
                    return;
                  }
                });
              ...
              console.log("getAllProjects error from sever!");
            });
        }, function() {
          console.log('Failed to get Projects from Server');
        });
      vm.xyz = true;
      vm.delete1 = function(id) {

      };
      vm.delete_project = function(id) {
        console.log("delete one project!" + id);
        if (id > -1) {
          vm.selected[id] = false;
          vm.deleted_ids.push(vm.projects[id]._id);
          vm.deleted_projects.push(vm.projects[id]);
          var temp_i = vm.added_ids.indexOf(vm.projects[id]._id);
          if (temp_i > -1) {
            vm.added_ids.splice(temp_i, 1);
            vm.added_projects.splice(temp_i, 1);
          }
        }
      }

      vm.add_project = function(id) {
        console.log("add one project!" + id);
        if (id > -1) {
          vm.selected[id] = true;
          vm.added_ids.push(vm.projects[id]._id);
          vm.added_projects.push(vm.projects[id]);
          var temp_i = vm.deleted_ids.indexOf(vm.projects[id]._id);
          if (temp_i > -1) {
            vm.deleted_ids.splice(temp_i, 1);
            vm.deleted_projects.splice(temp_i, 1);
          }
        }
      }

      vm.update_projects = function() {
        console.log("update projects!");
        vm.added_projects.forEach(function(project) {
          project.email = $localStorage.email;
          console.log(project);
          API.UpdateProjects($localStorage.session_id, project._id, project)
            .then(function(response) {
              console.log("updated project : " + response.data._id);
            }, function() {
              console.log("Failed Updating a project");
            });
        });
        vm.deleted_projects.forEach(function(project) {
          project.email = "";
          console.log(project);
          API.UpdateProjects($localStorage.session_id, project._id, project)
            .then(function(response) {
      ...
      // Saver User to DreamFactory
      vm.saveUser = function() {

        console.log(' Updating the User with Project Attributes');
        //console.log(vm.user);
        ...
        /*API.UpdateProjects(vm.user, $localStorage.session_id, $localStorage.dsp_id)
          .then(function() {
            console.log('Saving user from Form Controller');
            // return $http.post('/saveUser', data);
          }, function() {

            console.log('Error in Saving the Form Controller');
          });*/
...
(function() {
  ...
  LoginFormController.$inject = ['$http', '$state', 'API', '$localStorage'];

  function LoginFormController($http, $state, API, $localStorage) {
    var vm = this;
    ...
    function activate() {
      ...
      vm.login = function() {
        ...
        if (vm.loginForm.$valid) {
          $localStorage.email = vm.account.email;
          API.login(vm.account.email, vm.account.password)
            .then(function(response) {
              ...
              API.getDashboards($localStorage.session_id)
                .then(function(response) {
                  $localStorage.dashboards = response.data.resource.slice();
                  $localStorage.dashboards.forEach(function(d){
                    d._id = +d._id;
                  });
                }, function() {
                  vm.authMsg = 'Server Request Error(getDashboards)';
                });

              API.getDashboard($localStorage.session_id, user_id)
                .then(function(response) {
                  $localStorage.user_dashboards = [];
                  for (var i in response.data.dashboard_id) {
                    $localStorage.user_dashboards.push(response.data.dashboard_id[i]);
                  }
                  $state.go('app.dashboard_v' + $localStorage.user_dashboards[0]);
                }, function() {
                  vm.authMsg = 'Server Request Error(getDashboard)';
                });
...
(function() {
  ...
  function routesConfig($stateProvider, $locationProvider, $urlRouterProvider, helper) {
    ...
    $stateProvider
      ...
      .state('app.add_remove_dashboard', {
        url: '/add-remove-dashboard',
        templateUrl: helper.basepath('add-remove-dashboard.html'),
        resolve: helper.resolveFor('xeditable')
      })
...
(function() {
  ...
  function settingsRun($rootScope, $localStorage) {
    ...
    $rootScope.app = {
      name: 'ThingsIO',
      description: 'ThingsIoT Platfrom',
...
(function() {
  ...
  SidebarController.$inject = ['$rootScope', '$scope', '$state', 'SidebarLoader', 'Utils', '$localStorage', 'API'];

  function SidebarController($rootScope, $scope, $state, SidebarLoader, Utils, $localStorage, API) {

    activate();

    $rootScope.$on('CallLoadSidebar', function(){
      activate();
    })
    ...
    function activate() {
      ...
      function sidebarReady(items) {
        if ($localStorage == null || $localStorage.user_dashboards == null) { return; }

        API.getDashboard($localStorage.session_id, $localStorage.user_id)
        .then(function(response) {
          console.log(" sidebar load : ");
          console.log(response.data.dashboard_id);
          $localStorage.user_dashboards = [];
          for (var i in response.data.dashboard_id) {
            $localStorage.user_dashboards.push(response.data.dashboard_id[i]);
          }
        ...
        var i, len = $scope.menuItems.length,
          key1 = "text",
          key2 = "submenu",
          key3 = "alert";

        for (i = 0; i < len; i++) {
          if ($scope.menuItems[i] && $scope.menuItems[i].hasOwnProperty(key1)) {
            if ($scope.menuItems[i][key1] == "Dashboard" && $scope.menuItems[i].hasOwnProperty(key2)) {

              $scope.menuItems[i][key3] = $localStorage.user_dashboards.length;

              var d = {
                "text": "",
                "sref": ""
              };
              $scope.menuItems[i][key2] = [];

              for (var j = 0; j < Object.keys($localStorage.user_dashboards).length; j++) {
                d.text = "Dashboard v" + $localStorage.user_dashboards[j];
                d.sref = "app.dashboard_v" + $localStorage.user_dashboards[j];
                $scope.menuItems[i][key2].push(d);
                d = {
                  "text": "",
                  "sref": ""
                };
  ...
  function TablexEditableController($filter, $http, editableOptions, editableThemes, $q) {
    ...
    function activate() {
      ...
      vm.users = [{
        id: 1,
        name: 'user1',
        email: 'amruthp@thethingscloud.com',
        status: 2,
        group: 4,
        groupName: 'admin'
      }, {
...
/**=========================================================
 * Module: API Services
 * Factory API Services to Dream Factory
 =========================================================*/
// A RESTful factory for retrieving mails from json file
...
(function() {
  ...
  function API($http) {
    ...
    var DSP_URL = 'http://104.239.145.157',
      DSP_API_KEY = '36fda24fe5588fa4285ac6c6c2fdfbdb6b6bc9834699774c9bf777f706d05a88',
      userInfo = {
        email: "",
        password: "",
        sessionId: ""
    ...
    var ProjectX = {};
    var DSP_PUT_ID = '57d2fd215efc92d72cbd20cf';
    ...
    function login(email, password) {
      ...
      return $http({
        ...
        headers: {
          ...
          'X-DreamFactory-Application-Name': DSP_API_KEY
        },
        data: {
        ...
        userInfo.sessionId = data.session_id;
    ...
    function getData(deviceId, session_id) {

      console.log(userInfo.sessionId);

      return $http({
        method: 'GET',
        url: DSP_URL + '/api/v2/thingscloud2/_table/data',
        headers: {
          'Content-Type': 'Application/json',
    ...
    function getDashboard(session_id, user_id) {
      return $http({
        method: 'GET',
        url: DSP_URL + '/api/v2/thingscloud2/_table/dashboard/' + user_id,
        headers: {
          'Content-Type': 'Application/json',
          'X-Dreamfactory-API-Key': DSP_API_KEY,
          'X-DreamFactory-Session-Token': session_id
    ...
    function patchDashboard(session_id, user_id, dashboard_ids) {
      return $http({
        method: 'PATCH',
        url: DSP_URL + '/api/v2/thingscloud2/_table/dashboard/' + user_id,
        headers: {
    ...
    function getDashboards(session_id) {
      return $http({
        method: 'GET',
        url: DSP_URL + '/api/v2/thingscloud2/_table/dashboards/',
        headers: {
          'Content-Type': 'Application/json',
          'X-Dreamfactory-API-Key': DSP_API_KEY,
          'X-DreamFactory-Session-Token': session_id
    ...
    /*function refresh(session_id) {
      return $http({
        method: 'GET',
        url: DSP_URL + '/api/v2/thingscloud2/_table?refresh=true',
        headers: {
          'Content-Type': 'Application/json',
          'X-Dreamfactory-API-Key': DSP_API_KEY,
          'X-DreamFactory-Session-Token': session_id
